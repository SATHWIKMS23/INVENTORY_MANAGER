  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md px-4">
      <div className="bg-white/80 backdrop-blur-md p-6 sm:p-8 rounded-3xl shadow-2xl border border-gray-100">
        ...
        {error && <div className="mb-4 p-3 bg-red-100 text-red-700 text-xs sm:text-sm rounded-xl border-l-4 border-red-500">{error}</div>}

        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          <div className="space-y-2">
            <label className="text-xs sm:text-sm font-medium text-gray-700 ml-1">Email</label>
            <div className="relative group">
              <User className="absolute left-3 top-3 sm:top-3.5 h-5 w-5 text-gray-400 group-focus-within:text-blue-500" />
              <input
                type="email"
                required
                className="block w-full pl-10 pr-3 py-2 sm:py-3 border border-gray-200 rounded-xl bg-gray-50/50 outline-none focus:ring-2 focus:ring-blue-500 transition-all text-sm"
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <Lock className="absolute left-3 top-3 sm:top-3.5 h-5 w-5 text-gray-400 group-focus-within:text-blue-500" />
              <input
                type="password"
                required
                className="block w-full pl-10 pr-3 py-2 sm:py-3 border border-gray-200 rounded-xl bg-gray-50/50 outline-none focus:ring-2 focus:ring-blue-500 transition-all text-sm"
          ...
          <motion.button
            ...
            disabled={loading}
            type="submit"
            className="w-full flex justify-center items-center py-2 sm:py-3 rounded-xl shadow-lg text-white bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 gap-2 font-semibold text-sm sm:text-base"
          >
            {loading ? <Loader2 className="animate-spin w-4 h-4" /> : <>Sign In <ArrowRight className="w-4 h-4" /></>}
          </motion.button>
        </form>
